...
if grep -q "password.*requisite.*pam_pwhistory.so" "$PAM_FILE"; then
    ...
    sed -i '0,/^password/s/^password/password requisite pam_pwquality.so retry=3 minlen=12 difok=3 ucredit=-1 lcredit=-1 dcredit=-1 ocredit=-1 maxrepeat=2 gecoscheck=1 dictcheck=1 usercheck=1 enforcing=1\npassword/' "$PAM_FILE"
fi
